![](https://github.com/tarcisiobruni/aws-statemachine-workflow/blob/main/stepfunctions_graph.png)
...
- statemachines - Definition for the state machine that orchestrates the number generator.
...
```bash
aws-api-statemachine$ sam build
```
...
```bash
aws-api-statemachine$ sam logs -n GenNumberFunction --stack-name aws-api-statemachine --tail
```
...
```bash
aws-api-statemachine$ cd functions/num-gen
num-gen$ npm install
num-gen$ npm run test
```
...
```bash
aws cloudformation delete-stack --stack-name aws-api-statemachine
```
...
- aws s3 mb s3://api-statemachine-bucket --profile personal --region us-east-1
- sh deploy.sh 
